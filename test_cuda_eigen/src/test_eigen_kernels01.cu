#include "test_cuda_eigen/interface/test_eigen_kernels01.h"
...
//
__global__ void cu_eigen_madd(Matrix8x8* ma, 
                              Matrix8x8* mb, 
                              Matrix8x8* mc) {
    int id = blockIdx.x;
    mc[id] = ma[id] + mb[id];
...
void eigen_matrix_add(Matrix8x8* ma,
                      Matrix8x8* mb,
                      Matrix8x8* mc, int const n) {
...
//
__global__ void cu_eigen_mtests(Matrix8x8 *min,
                                Matrix8x8 *mout) {
    int id = blockIdx.x;
    // test transposition
    mout[id] = min[id].transpose();
    mout[id].row(7) = Eigen::Matrix<Matrix8x8::value_type, 1, 8>::Zero();

//    mout[id].llt();
//    Eigen::LLT<Matrix10x10> llt = mout[id].llt();
}

void eigen_matrix_tests(Matrix8x8 *min,
                        Matrix8x8 *mout,
                        int const n) {
    cu_eigen_mtests<<<n, 1>>>(min, mout);
}

__global__ void map_to_eigen_matrix(double* data, Matrix8x8* m, int size) {
  *m = Eigen::Map<Matrix8x8>(data, size, size);
}
